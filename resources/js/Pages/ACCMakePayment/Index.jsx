...
import Modal from '@/Components/CustomModal'; // Assuming a custom modal component
...
export default function Index({ auth, payments, filters, success }) {
    ...
    useEffect(() => {
        ...
        searchTimeoutRef.current = setTimeout(() => {
            router.get(route("accounting1.index"), data, { // Note: using accounting1 as per your routes
                preserveState: true,
                replace: true,
            });
        }, DEBOUNCE_DELAY);
                                        ...
                                        payments.data.map((payment) => (
                                            <tr key={payment.id} className="hover:bg-gray-50">
                                                <td className="whitespace-nowrap px-4 py-4 text-sm text-gray-800 font-medium">
                                                    {/* THE FIX IS HERE: Use `display_name` from the accessor */}
                                                    {payment.recipient?.display_name || 'N/A'}
                                                </td>
                                                ...
                                                <td className="whitespace-nowrap px-4 py-4 text-center text-sm">
                                                    <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getStageStyles(payment.stage)}`}>
                                                        {PAYMENT_STAGE_LABELS[payment.stage] || 'Unknown'}
                                                    </span>
                                                </td>
                                                <td className="whitespace-nowrap px-4 py-4 text-sm text-center">
                                                    <div className="flex items-center justify-center space-x-4">
                                                        <Link href={route("accounting1.edit", payment.id)} className="text-blue-600 hover:text-blue-800" title="Edit/View"><FontAwesomeIcon icon={faEdit} /></Link>
