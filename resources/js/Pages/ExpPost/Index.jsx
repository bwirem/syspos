...
export default function Index({ auth, posts, filters }) {
    ...
    useEffect(() => {
        get(route("expenses0.index"), { preserveState: true });
    ...
    const handleModalConfirm = async () => {
        try {
            await router.delete(route("expenses0.destroy", modalState.postToDeleteId));
        } catch (error) {
            console.error("Failed to delete post:", error);
            showAlert("There was an error deleting the post. Please try again.");
        }
        setModalState({ isOpen: false, message: '', isAlert: false, postToDeleteId: null });
    };
    ...
    // Map post stage numbers to labels
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold text-gray-800">Expense List</h2>}
        >
            ...
            <div className="container mx-auto p-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                    ...
                    <ul className="flex space-x-2 mt-2">
                         ...
                         {Object.entries(postStageLabels).map(([key, label]) => (
                            <li
                                key={key}
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full border border-gray-300 shadow-md rounded">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="border-b p-3 text-center font-medium text-gray-700">Post Descriptions</th> 
                            ...
                            {posts.data.length > 0 ? (
                                posts.data.map((post, index) => (
                                    <tr key={post.id} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                                        <td className="border-b p-3 text-gray-700">{post.description ? post.description : "n/a"}</td>                                        
                                        ...
                                        <td className="border-b p-3 text-gray-700">{postStageLabels[post.stage]}</td>
                                ...
                                <tr>
                                    <td colSpan="4" className="border-b p-3 text-center text-gray-700">No posts found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
