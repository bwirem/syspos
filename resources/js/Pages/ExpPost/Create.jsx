...
export default function Create({facilityoption}) {
    ...
    const addPostItem = (selectedItem = null) => {
        ...
        setPostItems((prevItems) => [...prevItems, newItem]);
        setItemSearchQuery('');
        setItemSearchResults([]);
        setShowItemDropdown(false);
    };
    ...
    const handleSubmit = (e) => {
        ...
        const hasEmptyFields = postItems.some(
            (item) => !item.item_name || !item.item_id || item.amount <= 0
        );
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">New Expense</h2>}
        >
