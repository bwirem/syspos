...
export default function SaveOrderConfirmation({ auth, orderData }) {
    ...
    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Confirm and Save Order</h2>}>
                    ...
                    <div className="overflow-hidden bg-white dark:bg-gray-800 p-6 shadow-sm sm:rounded-lg">
                        <form onSubmit={submitOrder} className="space-y-6">                           
                            ...
                            <section className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                                ...
                                <div>
                                    <label htmlFor="stage" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Save order as</label>
                                    <select id="stage" value={data.stage} onChange={(e) => setData('stage', e.target.value)} className="w-full mt-1 border p-2 rounded text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">
