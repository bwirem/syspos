...
export default function Index({ auth, items, filters }) {
    ...
    useEffect(() => {
        get(route("systemconfiguration1.items.index"), { preserveState: true });
    ...
    const handleModalConfirm = async () => {
        try {
            await router.delete(route("systemconfiguration1.items.destroy", modalState.itemToDeleteId));
        } catch (error) {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold text-gray-800">Expenses List</h2>}
        >
            ...
            <div className="container mx-auto p-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                    <div className="flex items-center space-x-2 mb-4 md:mb-0">
                        ...
                        <Link
                            href={route("systemconfiguration1.items.create")}
                                ...
                                items.data.map((item, index) => (
                                    <tr key={item.id} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                                        ...
                                        <td className="border-b p-3 flex space-x-2">
                                            <Link
                                                href={route("systemconfiguration1.items.edit", item.id)}
