...
import { faFilter, faRedo, faCalendarTimes, faSpinner, faExclamationTriangle } from '@fortawesome/free-solid-svg-icons';
...
export default function ExpiringItemsReport({ auth, expiringItems = [], stores = [], productsList = [], filters = {}, flash }) {
    const { data, setData, get, processing, errors, reset } = useForm({
        store_id: filters.store_id || '',
        product_id: filters.product_id || '',
        days_to_expiry: filters.days_to_expiry_applied || '30', // Default to 30 days, match controller's applied filter
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        get(route('reports.inventory.expiring_items'), { // Ensure this route name is correct
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const resetFilters = () => {
        reset(); // Resets to initial values from useForm (which includes filters prop)
        // Or reset to hardcoded defaults:
        // setData({
        //     store_id: '',
        //     product_id: '',
        //     days_to_expiry: '30',
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Expiring Items Report</h2>}
        >
            <Head title="Expiring Items Report" />
                    ...
                    <div className="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                        <form onSubmit={handleSubmit} className="mb-8 space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4 items-end">
                                ...
                                <div>
                                    <label htmlFor="days_to_expiry" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Expiring in (Days)</label>
                                    <input
                                        type="number"
                                        name="days_to_expiry"
                                        id="days_to_expiry"
                                        value={data.days_to_expiry}
                                        ...
                                        placeholder="e.g., 30"
                                    />
                                    {errors.days_to_expiry && <p className="text-xs text-red-500 mt-1">{errors.days_to_expiry}</p>}
                                </div>
                        ...
                        {processing && (
                             <div className="text-center py-10 text-gray-500 dark:text-gray-400">
                                ...
                                <p className="mt-2">Loading Expiring Items Data...</p>
                            </div>
                        )}

                        {!processing && expiringItems && expiringItems.length > 0 ? (
                                ...
                                <table className="min-w-full divide-y dark:divide-gray-600">
                                    <thead className="bg-gray-50 dark:bg-gray-700/50">
                                        <tr>
                                            ...
                                            <th className={thDefaultClass}>Batch No.</th>
                                            <th className={`${thDefaultClass} text-right`}>Quantity</th>
                                            <th className={`${thDefaultClass} text-center`}>Expiry Date</th>
                                        </tr>
                                    </thead>
                                    <tbody className="bg-white dark:bg-gray-800 divide-y dark:divide-gray-700">
                                        {expiringItems.map((item) => (
                                            <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                <td className={tdDefaultClass}>{item.product?.name || 'N/A'}</td>
                                                <td className={tdDefaultClass}>{item.store?.name || 'N/A'}</td>
                                                <td className={tdDefaultClass}>{item.butchno || 'N/A'}</td>
                                                <td className={`${tdDefaultClass} text-right`}>{item.quantity}</td>
                                                <td className={`${tdDefaultClass} text-center`}>{formatDate(item.expirydate)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            !processing && <p className="text-center text-gray-500 dark:text-gray-400 mt-8 py-10">No items found expiring soon for the selected criteria.</p>
                        )}
