...
// Stock Selection Modal Component (Same as Create)
...
export default function Edit({ order, fromstore, auth, priceCategories, facilityOptions }) {
    // Correctly initialize items, matching structure from BilPosts/Edit
    const initializeOrderItems = (items) => {
        ...
        return items.map((item, index) => {
            ...
            return {
                ...
                // Assuming stock_quantity is not sent in initial load, keep undefined to skip validation on existing lines
    ...
    useEffect(() => { if (itemSearchQuery.trim()) debouncedSearch(itemSearchQuery); else setItemSearchResults([]); }, [itemSearchQuery, debouncedSearch]);
    ...
    const handleAlternativeStoreSelect = (storeId) => {
        ...
        if (store && pendingItem) {
             axios.get(route('systemconfiguration0.items.search'), { 
                params: { query: pendingItem.name, store_id: store.id, pricecategory_id: data.pricecategory_id } 
            })
            .then(response => {
                const exactItem = response.data.items.find(i => i.id === pendingItem.id);
                if (exactItem) addItemToCart(exactItem, store.id, store.name);
                else addItemToCart(pendingItem, store.id, store.name);
                setShowStockModal(false); setPendingItem(null); setAvailableAlternativeStores([]);
            })
            .catch(error => { console.error(error); addItemToCart(pendingItem, store.id, store.name); setShowStockModal(false); });
        }
    };
    ...
    const handleOrderItemChange = (index, field, value) => {
        setOrderItems(currentItems => {
            ...
            if (isNaN(parsedValue) || parsedValue < 0) parsedValue = field === 'quantity' ? 1 : 0;
    ...
    const removeOrderItem = (index) => {
        setModalState({ isOpen: true, message: 'Are you sure you want to remove this item?', isAlert: false, itemToRemoveIndex: index });
    };
    ...
    // Search Handlers
    const handleItemSearchChange = (e) => { setItemSearchQuery(e.target.value); setShowItemDropdown(!!e.target.value.trim()); };
    ...
    const handleCustomerSearchChange = (e) => { setCustomerSearchQuery(e.target.value); setCustomerSearchResults([]); setShowCustomerDropdown(!!e.target.value.trim()); setData(prev => ({ ...prev, first_name: '', other_names: '', surname: '', company_name: '', email: '', phone: '', customer_id: null })); };
    const handleClearCustomerSearch = () => { setCustomerSearchQuery(''); setCustomerSearchResults([]); setShowCustomerDropdown(false); if(customerSearchInputRef.current) customerSearchInputRef.current.focus(); setData(prev => ({ ...prev, first_name: '', other_names: '', surname: '', company_name: '', email: '', phone: '', customer_id: null })); };

    const selectCustomer = (selectedCustomer) => {
        setData(prev => ({ ...prev, customer_type: selectedCustomer.customer_type, first_name: selectedCustomer.first_name || '', other_names: selectedCustomer.other_names || '', surname: selectedCustomer.surname || '', company_name: selectedCustomer.company_name || '', email: selectedCustomer.email, phone: selectedCustomer.phone || '', customer_id: selectedCustomer.id }));
    ...
    const handleSaveClick = () => {   
       ...
       if (data.orderitems.length === 0) { toast.error('Please add at least one item before saving.'); return; }        
   ...
   const handleSaveModalConfirm = () => {        
       ...
       put(route('billing0.update', order.id), formData, {
           forceFormData: true,
           onSuccess: () => { setSaveModalLoading(false); reset(); setSaveModalSuccess(true); handleSaveModalClose(); toast.success('Order Saved'); },
    ...
    const handleSubmitClick = () => {
         if (!isValidInteger(data.customer_id)) { setCustomerIDError('Customer ID must be a valid integer.'); return; } else { setCustomerIDError(null); }
         if (!isValidInteger(data.store_id)) { setStoreIDError('Store ID must be a valid integer.'); return; } else { setStoreIDError(null); }
        if (data.orderitems.length === 0) { toast.error('Please add at least one item before submitting.'); return; } 
        setData('stage', 3);
        setSubmitModalOpen(true); setSubmitModalLoading(false); setSubmitModalSuccess(false);
    };

    const handleSubmitModalClose = () => { setData('stage', order.stage); setSubmitModalOpen(false); setSubmitModalLoading(false); setSubmitModalSuccess(false); };
    ...
    const handleSubmitModalConfirm = () => {        
        ...
        put(route('billing0.update', order.id), formData, {
            forceFormData: true,
            onSuccess: () => { setSubmitModalLoading(false); reset(); setSubmitModalSuccess(true); handleSubmitModalClose(); toast.success('Order Submitted'); },
            onError: (errors) => { setSubmitModalLoading(false); console.error(errors); toast.error('Error submitting order'); },
        });
    };

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Edit Order</h2>}>
            <Head title="Edit Order" />
            <div className="py-12">
                <div className="mx-auto max-w-4xl sm:px-6 lg:px-8">
                    <div className="bg-white p-6 shadow sm:rounded-lg">
                        <form className="space-y-6" onSubmit={e => e.preventDefault()}>
                            {/* ... Customer Section (Unchanged, copy layout from Create.jsx if needed for consistency) ... */}
                           ...
                           <section className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                                ...
                                <div className="mb-4">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Add Item to Order</label>
                                                    ...
                                                    {orderItems.map((item, index) => (
                                                        <tr key={index}>
                                                            <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-700 dark:text-gray-200">
                                                                <div className="font-medium">{item.item ? item.item.name : item.item_name}</div>
                                ...
                                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex justify-end">
                                    <div className="w-full md:w-1/3">
                                        ...
                                        <div className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100 bg-gray-100 dark:bg-gray-700 p-3 rounded-md text-right">TZS {formatCurrency(data.total)}</div>
                                    </div>
                                </div>
                            </section>

                           <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                                <Link href={route('billing0.index')} className="px-4 py-2 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500 text-sm flex items-center"><FontAwesomeIcon icon={faTimesCircle} className="mr-2" /> Close</Link>
                                ...
                                <button type="button" onClick={handleSubmitClick} disabled={processing} className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 disabled:opacity-50 text-sm flex items-center"><FontAwesomeIcon icon={processing ? faSpinner : faCheck} spin={processing} className="mr-2" /> Submit</button>
                            </div>
                        </form>
                    </div>
                </div>
