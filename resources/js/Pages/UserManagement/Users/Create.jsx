...
export default function Create() {
    ...
    // Fetch user groups
    useEffect(() => {
        axios.get(route('usermanagement.usergroups.search'))
            .then(response => setUserGroups(response.data.groups))
            .catch(() => showAlert('Failed to fetch user groups.'));
    }, []);
                    ...
                    <div className="bg-white p-6 shadow sm:rounded-lg">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">                              
                                ...
                                <div className="relative flex-1 flex users-center">
                                    <input
                                        id="addtocart"
                                        type="checkbox"
                            ...
                            <div className="flex justify-end space-x-4">
                                <button type="button" onClick={() => Inertia.get(route('usermanagement.users.index'))} className="bg-gray-300 text-gray-700 rounded p-2 flex users-center space-x-2">
                                    <FontAwesomeIcon icon={faTimesCircle} />
                                    <span>Cancel</span>
                                </button>
                                <button type="submit" disabled={processing || isSaving} className="bg-blue-600 text-white rounded p-2 flex users-center space-x-2">
