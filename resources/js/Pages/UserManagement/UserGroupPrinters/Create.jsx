...
export default function Create({ usergroups }) {
    const { data, setData, post, errors, processing, reset } = useForm({
        usergroup_id: '',
        machinename: '', // Can default to window.location.hostname if desired
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        setIsSaving(true);
        post(route('usermanagement.usergroupprinters.store'), {
            onSuccess: () => {
                setIsSaving(false);
                reset();
                showAlert('Printer configuration created successfully!');
            },
            onError: () => {
                setIsSaving(false);
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-gray-800">New Printer Config</h2>}>
            <Head title="New Printer Config" />
            <div className="py-12">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        className={`w-full border p-2 rounded text-sm ${errors.documenttypecode ? 'border-red-500' : ''}`}
                                    >
